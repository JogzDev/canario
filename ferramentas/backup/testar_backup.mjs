...
try {
  await origem.exec(`
    ...
    ALTER TABLE public.pecas DROP COLUMN coluna_removida;
    CREATE TYPE public.tipo_com_lacuna AS (a integer, removido text, b text);
    ALTER TYPE public.tipo_com_lacuna DROP ATTRIBUTE removido;
  ...
  console.log('ok 1d: lacunas físicas de DROP COLUMN/ATTRIBUTE não alteram contrato lógico');
  const original = await readFile(path.join(dest,'manifesto.json'),'utf8');
  const wrongCatalog = JSON.parse(original);
  const columns = wrongCatalog.catalog.relations.find(r=>r.schema==='public' && r.name==='pecas').columns;
  [columns[0],columns[1]] = [columns[1],columns[0]];
  await writeFile(path.join(dest,'manifesto.json'),JSON.stringify(wrongCatalog));
  await assert.rejects(restore(bin,dest),/Catálogo restaurado diverge/);
  console.log('ok 1e: mudança de ordem lógica das colunas continua reprovada');
