-- Asserções da A64 da cobertura (20260924013645): a marca da Nuvemshop é
-- cobrada pelo portão de publicação. Duas migrations nasceram com o rótulo
-- A64 na mesma noite; esta é a da cobertura, a outra é a da leitura.
--
-- O `rodar.mjs` roda este arquivo duas vezes: antes da migration, numa
-- transação desfeita, e exige que ele REPROVE (a A60 deixa a Amaro fora da
-- coorte); depois dela, e exige que passe. A Amaro da fixture da A61 (marca 7)
-- entra como está em produção desde 23/09: `nuvemshop`.

-- 59. Amaro sem coleta no dia trava a publicação; com coleta, está coberta.
do $$
declare c record; begin
  ...
  raise notice 'ok 59 depois: a Amaro na Nuvemshop entra na coorte: sem coleta trava, com coleta publica';
end $$;
