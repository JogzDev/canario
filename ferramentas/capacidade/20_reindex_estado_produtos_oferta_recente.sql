-- PASSO 20 · DEVOLVE ESPACO FISICO.
--
-- @espera_encolher
-- @alvo public.estado_produtos_oferta_recente
--
-- Alvo exato: o indice `public.estado_produtos_oferta_recente` e nada mais.
--
-- O que faz: reconstroi o indice do zero. O dado da tabela nao muda.
-- Medido em 18/09: 9.093.120 bytes; um btree recem-construido com as mesmas entradas
-- teria ~1,94 MB (61.694 entradas) (formula do inventario, consulta 4). Ganho estimado: ~7,2 MB. E o indice parcial que toda coleta reescreve: cada `ultimo_avistamento_em` novo move a entrada.
--
...
-- @acao
reindex index public.estado_produtos_oferta_recente;
