...
as $function$
  -- LIMIAR DE SEMELHANCA: 70% dos atributos marcados, arredondado para cima.
  --
  -- A primeira versao exigia "todos menos um" e devolveu 3.989 similares para
  -- "vestido + floral + midi", porque com tres atributos tolerar uma diferenca
  -- deixa entrar todo vestido midi liso e todo floral curto. Medido: 230 tem os
  -- TRES. Chamar as outras 3.759 de "parecidas" esvazia a palavra.
  --
  --     2 atributos -> exige 2      5 atributos -> exige 4
  --     3 atributos -> exige 3      6 atributos -> exige 5
  --     4 atributos -> exige 3      8 atributos -> exige 6
  --
  -- O FORMATO CARO SO RODA NA AMOSTRA.
  --
  -- `grade_em_texto()` faz um lateral com `ordem_do_tamanho()` por tamanho, e
  -- chama-la em todos os similares custava 11,3 SEGUNDOS numa busca por
  -- "vestido" sozinho (11.579 pecas) -- quase quatro vezes o limite de 3s do
  -- papel `anon`, que e o que o app usa. O agregado precisa apenas saber SE a
  -- grade esta quebrada, e isso e um `bool_or` direto no jsonb. O texto
  -- formatado so e montado para as ~12 pecas que viram cartao.
  with parametros as (
  ...
  sim as (
    select p.id, p.titulo, p.url, p.ultimo_preco_atual as preco,
           ...
           -- Barato: so pergunta se ALGUM tamanho esta indisponivel.
           (select bool_or(value = 'false'::jsonb)
              from jsonb_each(coalesce(p.ultima_grade, '{}'::jsonb))) as quebrada,
           not exists (select 1 from jsonb_each(coalesce(p.ultima_grade, '{}'::jsonb))
                       where value = 'true'::jsonb) as esgotada
    from candidatos c
  ...
  amostra as (
    select jsonb_agg(jsonb_build_object(
      ...
      'em_comum', em_comum,
      'grade', public.grade_em_texto(ultima_grade)) order by em_comum desc, id) as j
