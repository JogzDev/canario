...
as $function$
  ...
  -- AMOSTRA ESPALHADA PELAS MARCAS, e nao as primeiras por id.
  --
  -- Ordenar por `em_comum desc, id` devolveu tres pecas da PatBo, todas
  -- esgotadas e com 50-70% de desconto -- num conjunto de 230 similares em 9
  -- marcas, onde 22% estao a preco cheio. A amostra contava outra historia que
  -- o resumo. Agora vem a peca mais parecida de CADA marca primeiro, depois a
  -- segunda de cada, e assim por diante: o comprador ve quem mais esta fazendo
  -- aquilo, que e a pergunta que o §29 responde.
  ordenado as (
  ...
  amostra as (
    ...
    from (select * from ordenado order by posicao_na_marca, em_comum desc, id limit limite) t
  )
  select jsonb_build_object(
