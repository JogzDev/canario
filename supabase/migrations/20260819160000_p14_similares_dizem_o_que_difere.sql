-- P14: cada peça semelhante passa a dizer QUAIS atributos ela tem em comum.
--
-- POR QUE
-- =======
--
-- O P13 fez as peças que batem em tudo liderarem o bloco. Mas o bloco continua
-- mostrando casamentos parciais depois delas -- a regra dos 70% permite -- e o
-- app não avisa quais. Quem enviou uma camiseta listrada e vê uma peça lisa na
-- lista conclui, com razão, que o app errou.
--
-- Ele não errou: mostrou a terceira peça mais parecida que existe no painel. O
-- defeito é não dizer em que ela difere. Isso é a regra 2 do projeto aplicada
-- aqui -- não afirmar mais do que o dado sustenta.
--
-- `em_comum` (a contagem) já era devolvido e o app o descartava. Contagem sozinha
-- responde "quanto", não "o quê": "3 de 4" não diz se o que faltou foi a estampa
-- ou o tecido, e é a estampa que a pessoa enxerga na foto.
--
-- Agora vai também `termos_em_comum`, a lista. O app subtrai do que foi pedido e
-- nomeia a diferença com o rótulo já traduzido da taxonomia.
--
-- Custo: até 12 identificadores curtos por peça, no máximo 24 peças. Desprezível
-- ao lado da URL da imagem que já viaja em cada uma.
--
-- Reverter: tirar `termos_em_comum` do `candidatos`, do `sim` e do
-- `jsonb_build_object` da amostra.
...
as $function$
  with entrada as (
    ...
    select pt.produto_id,
           ...
           -- P14: a lista, e nao so a contagem. E ela que deixa o app dizer
           -- "esta nao e listrada" em vez de "3 de 4".
    ...
    -- P13: `em_comum` na frente; a variedade de marca desempata.
...
comment on function public.similares_da_peca(text[], integer, numeric) is
  'Peças semelhantes por atributos em comum. P13: ordena por semelhança, marca desempata. P14: devolve termos_em_comum, para o app poder nomear em que a peça difere em vez de só contar.';
