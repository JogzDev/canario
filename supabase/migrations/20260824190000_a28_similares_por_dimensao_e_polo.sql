-- A28 — similares sem o precipício dos 70%
--
-- Uma peça bicolor não deveria precisar encontrar títulos que nomeiem as duas
-- cores. Os termos escolhidos dentro da mesma dimensão passam a ser
-- alternativas para o casamento, enquanto dimensões diferentes continuam
-- acumulando evidência. A categoria continua obrigatória.

update public.termos
set sinonimos = case
      when coalesce(sinonimos, '') = '' then 'polo'
      when sinonimos !~* '(^|\|)polo($|\|)' then sinonimos || '|polo'
      else sinonimos end,
    palavras_pt = case
      when coalesce(palavras_pt, '') = '' then 'polo'
      when palavras_pt !~* '(^|\|)polo($|\|)' then palavras_pt || '|polo'
      else palavras_pt end,
    palavras_en = case
      when coalesce(palavras_en, '') = '' then 'polo'
      when palavras_en !~* '(^|\|)polo($|\|)' then palavras_en || '|polo'
      else palavras_en end
where id = 'camisa' and status = 'aprovado';
...
as $function$
  with entrada as (
    ...
    -- Usa o nível mais estrito que devolve alguma coisa. Só desce quando a
    -- alternativa é uma tela vazia; nunca relaxa a categoria.
...
comment on function public.similares_da_peca(text[], integer, numeric) is
  'A28: categoria obrigatoria; OR dentro da dimensao; relaxamento declarado apenas quando o nivel de 70% fica vazio.';
