...
| A15 | OpenAI no runtime: visão + redator com coleira | 🟡 Visão integrada atrás de `REMOTE_ANALYSIS_ENABLED`: Edge Function implantada, rate limit, contrato v5, disclosure, recorte obrigatório e fallback local compilam. Faltam secrets/migração, teste real e redator com coleira |
| A16 | Interface da v1 em inglês | 🟡 Interface principal e lógica apresentável traduzidas; nome definido como **DataDrobe**. Falta uma revisão humana completa no device e String Catalog antes da loja |
| A17 | Calibrar Luna nas mesmas 24; só então abrir holdout de 300 | 🟡 **Portão aberto e gasto autorizado pelo JP:** categoria 20/24 (83,3%), cor 22/24 (91,7%). O benchmark de 300 agora exige segmentação Vision sem fallback bruto e exclui as 24 de calibração; execução remota pendente |
...
| A24 | Confirmação visual da peça-alvo e portão de capacidade do banco | 🟡 App oferece instâncias locais, crop/zoom 3:4 e descrição opcional do alvo; só a imagem confirmada e segmentada alimenta cor, Luna e miniatura. Banco em 467.963.027 bytes (93,59%), com bloqueio de coleta em 96%. Falta aceite físico e plano de retenção |
| A25 | Corte de submissão: Luna segura, inglês e App Store readiness | 🟡 DataDrobe 1.0 (1), inglês, testes e Archive Release prontos. Bloqueiam: secrets/migração/teste Luna, profile App Store, URLs/metadata e aceite TestFlight. Checklist em `APP_STORE_READINESS_14-08-2026.md` |
...
|---|---|---|---|
| NQ1 | **Tendências realmente semanais e frescas.** “Semana de 13/07” e manchetes de junho não podem ocupar a vitrine em agosto | 🟡 O Google está em 03/08 para 40 termos; o atraso aparente vinha da tela exigir estado composto. A vitrine agora separa pulso Google, editorial atual e movimentos confirmados. Coletor cobra 03/08 a partir de quinta e não consulta quando tudo está em dia | SLA de publicação semanal; alerta se categoria principal passar de 14 dias; pipeline A21 publicado e verde; nenhuma novidade exibida com mais de 21 dias |
| NQ2 | **Precisão editorial e caso Vini Jr.** “Camisa 7” de futebol não é moda | 🟡 Toda categoria passou a exigir que o título declare peça ou intenção de moda; Vini, gravidez, viagem, UFC e metáfora empresarial são regressões. A coleta atual materializa zero para sobrescrever falso positivo da semana | Amostra cega de pelo menos 100 matches: precisão ≥95% e recall ≥90%; depois reprocessar o período afetado e provar que o artigo não move série nem card |
...
| NQ8 | **Peça do usuário limpa, sem fundo, no Closet e na home** | 🟡 A24 isola instâncias com Vision, oferece crop/zoom e dica textual e corrige a medição para ignorar pixels transparentes. A selecionada alimenta análise e miniatura; imagem bruta não entra no benchmark de 300 | Ensaio no iPhone com pelo menos 24 fotos (isolada, corpo, cabide e conjunto), taxa de recorte aceitável registrada e foto completa sempre disponível |
...
| NQ12 | **Remover placeholders e fala robótica em todo o app** | 🟡 O placeholder do Explorar e os seis alertas falsos do menu saíram; as telas do menu agora têm conteúdo ou função real. Ainda há strings legadas em português fora desse fluxo | Inventário restante de strings e fechamento junto da NQ15; elemento não implementado não aparece como se funcionasse |
| NQ13 | **Telas do menu: Favorites, Account, Terms, Settings, Privacy e Q&A** | 🟡 Seis destinos reais em inglês: Favorites persistente com foto/relatório, Account honesto sobre estado local, termos operacionais, exclusão do Closet em Settings, Privacy aderente ao binário atual e Q&A do método. 136 testes + build SwiftUI verde | Revisão jurídica humana de Terms/Privacy antes da submissão; quando conta ou OpenAI entrarem no runtime, os dois textos e a ficha da App Store precisam mudar no mesmo commit |
...
| NQ15 | **Interface final em inglês e nome do produto** | 🟡 Nome definido como **DataDrobe**; purpose strings, telas principais, relatórios, comparação, curvas, tendências e taxonomia apresentável passaram para inglês, com testes atualizados | Revisão humana no device, String Catalog, datas/locales e screenshots antes da submissão |
| NQ16 | **Luna: repetir 24 e integrar visão/redação só depois do portão** | 🟡 v5: **20/24 categoria, 22/24 cor**. Runtime tem contrato, consentimento, recorte, privacidade, rate limit e fallback; Edge implantada mas ainda sem secrets/migração. Holdout de 300 autorizado e preparado | Concluir backend/teste real; executar e adjudicar as 300; redator separado com validador de fatos |
