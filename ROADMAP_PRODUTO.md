...
| OP-09 | Tornar CI e execução reproduzíveis e isolados | M / U1 | parcial | Inventário de workflows/runners | CI Python e Apple já são efêmeros/gerenciados; a execução de dados permanece pausada até OP-12. Dependências fixadas, cache, permissões mínimas, secrets só no ambiente correto, jobs sem dependência acidental da sessão pessoal; fila e cancelamentos testados |
| OP-10 | Preservar o legado do i7 antes da devolução | M / U0 | concluído | Acesso ao runner `sempre-ligado`; destino privado depois do inventário | Inventário agregado sem conteúdo/segredo; cache externo copiado com manifesto e extração testada; último SHA e artefatos registrados antes de revogar o host |
| OP-11 | Provar egresso de executor gerenciado por classe de fonte | M / U0 | concluído para GitHub | Runner Linux hospedado disponível | Sonda manual, sem segredo/escrita, respeita robots e cadência global; quatro classes válidas e BoF bloqueado por robots 403; o veredito não autoriza Cloud Run sem nova prova no destino |
...
| OP-13 | Migrar build/testes Apple para Xcode Cloud | M / U0 | concluído | Conta/equipe Apple e quota Xcode Cloud | Scheme/config de CI e UI no Xcode 27 verdes sem runner pessoal; workflow só de Test; carga Vision manual separada; quota deve continuar observada |
| OP-14 | Desativar runners legados e rotacionar credenciais | M / U0 | corte concluído; rotação pendente | OP-10 e pausa segura; OP-12/13 para retomada completa | Agendas pausadas, runners 21/22 e variável removidos, LaunchAgent desinstalado, API/fila zeradas. Rotação das chaves requer inventário e dupla chave para não interromper consumidores de produção |
...
| OP-10 | Concluído: runs `35138670263`/`35138670359`; 5.289 JPG preservados num draft privado, SHA-256 registrado e restauração externa integral verificada. |
| OP-11 | Concluído para a origem GitHub: run `35138670304`; VTEX, Shopify, FFW e Trends válidos, BoF bloqueado por robots 403; `nao_autoriza_migracao: true` preservado. |
| OP-13/14 | Xcode Cloud build 2 verde no Xcode 27; runners 21/22, LaunchAgent e `RUNNER_COLETA` removidos; três agendas pausadas; zero runner e zero fila. Rotação segura das chaves continua pendente do inventário de consumidores. |
