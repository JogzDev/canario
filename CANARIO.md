...
- `VERIFICAÇÃO` | **Implementação local, ainda não aplicada em produção.** O checkpoint `pre-mudanca-20260820-presenca-oferta` preserva o estado anterior. As 27 suítes Python passaram, incluindo sellers VTEX múltiplos, preço sem estoque, Shopify de tamanho único, batimento sem snapshot diário e a transição de disponibilidade desconhecida do legado. A aplicação exige migração primeiro, coleta completa depois e só então republicação/medição antes-versus-depois.
