...
### 31/08/2026 — radar de mercado nasce evidence-first e separado da 1.2 (A51)

- `ADITIVA` | **A51 — inteligência de mercado é um domínio paralelo.** Conceitos, fontes, itens, execuções, evidências e leituras da 1.3 não ampliam `termos`, `produto_termos` nem `series_semanais`. O primeiro ciclo é privado e não altera índice, estado, z-score, raridade ou pipeline da 1.2.
- `ADITIVA` | **Descoberta não é evidência.** O Scout Luna trabalha somente dentro de allowlist autorizada, devolve no máximo oito itens `candidate_only`, usa `store=false` e nunca publica. `store=false` não significa retenção zero: a fundação inicial aceita somente o contrato padrão de até 30 dias; MAM/ZDR exigem futura revisão do schema e verificação administrativa do projeto. Snippet, release, lançamento e indisponibilidade não comprovam adoção; toda evidência nasce `draft` e exige decisão humana explícita.
- `ADITIVA` | **Direito de fonte precede capacidade técnica.** API, página pública ou chave disponível não liberam coleta, IA ou exibição. O registro executável separa status, método, processamento por IA, exibição e retenção; ausência, expiração ou conflito falham fechados. Pinterest e scraping social permanecem bloqueados, e as demais candidatas aguardam autorização específica.
- `ADITIVA` | **Cadência do piloto mede antes de prometer.** Coleta privada é semanal, briefing revisado é quinzenal e a metodologia é reavaliada mensalmente. As primeiras quatro semanas são laboratório; as primeiras 12 edições exigem revisão integral. O custo de modelo é registrado por execução, mas não substitui cobertura, independência nem revisão.
- `ADITIVA` | **Luna tem funções não intercambiáveis.** Scout pode buscar apenas em fonte com processamento integral expressamente autorizado. Redator não navega: recebe fatos minimizados e evidências aprovadas, e cada frase precisa ser remontável aos IDs recebidos. Falha de validação usa fallback determinístico, sem repetição paga automática.

### 01/09/2026 — primeiro pacote factual privado da Etiqueta (A52)

- `ADITIVA` | **A52 — composição transitória vira fato candidato, não verdade publicada.** Uma RPC privada pagina apenas produtos brasileiros ofertáveis e vistos nos últimos sete dias, com a etiqueta mais recente ainda dentro dos 21 dias de snapshots. O materializador local guarda hash, trechos curtos, fatos, cobertura e abstensão; não reintroduz descrição de marketing e não grava no banco.
- `ADITIVA` | **Denominador observável fica separado da população.** O share de algodão, malha ou acabamento usa os produtos em que a respectiva dimensão foi reconhecida; a cobertura da dimensão contra todos os elegíveis aparece ao lado. Ausência de etiqueta nunca vira ausência do material.
- `ADITIVA` | **Laboratório continua isolado.** O workflow é manual, sem IA e sem escrita, o artefato privado expira em sete dias e nenhuma linha é promovida a conceito, evidência ou briefing sem revisão humana.
